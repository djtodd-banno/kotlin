...
class JsIrAstSerializer(private val pathResolver: (File) -> String) {
    ...
    private fun serialize(statement: JsStatement): Statement {
        val visitor = object : JsVisitor() {
            ...
            override fun visitSingleLineComment(comment: JsSingleLineComment) {
                builder.singleLineComment = SingleLineComment.newBuilder().setMessage(comment.text).build()
            }
        }
        ...
        if (visitor.builder.statementCase == Statement.StatementCase.STATEMENT_NOT_SET) {
