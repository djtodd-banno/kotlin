...
static size_t snprintf_with_addr(char* buf, size_t size, size_t frame, const void* addr, bool is_inline, const char *format, ...) {
    ...
    if (info.dli_sname) {
        ...
        symbol_offset = (ptrdiff_t)addr - (ptrdiff_t)info.dli_saddr;
    } else if (info.dli_fname) {
        symbol = image;
        symbol_offset = (ptrdiff_t)addr - (ptrdiff_t)info.dli_fbase;
    } else if (0 < konan::snprintf(symbuf, sizeof(symbuf), "%p", info.dli_saddr)) {
        symbol = symbuf;
        symbol_offset = (ptrdiff_t)addr - (ptrdiff_t)info.dli_saddr;
    } else {
        symbol_offset = (ptrdiff_t)addr;
    }
#else // __has_include("dlfcn.h")
    char symbol_[512];
