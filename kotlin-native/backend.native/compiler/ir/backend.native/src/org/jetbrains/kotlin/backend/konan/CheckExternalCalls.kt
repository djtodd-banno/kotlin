        ...
        for (call in calls) {
            ...
            when (calleeInfo.name) {
                "objc_msgSend" -> {
                    ...
                    val firstArgClassPtr = LLVMBuildCall(builder, getClass.llvmFunction, listOf(firstArgI8Ptr).toCValues(), 1, "")
                    ...
                    val calledPtrLlvmIfNotNilFunPtr = LLVMBuildCall(builder, getMethodImpl.llvmFunction, listOf(firstArgClassPtr, selector).toCValues(), 2, "")
                ...
                "objc_msgSendSuper2" -> {
                    ...
                    val classPtr = LLVMBuildCall(builder, getSuperClass.llvmFunction, listOf(superClassPtr).toCValues(), 1, "")
                    val calledPtrLlvmFunPtr = LLVMBuildCall(builder, getMethodImpl.llvmFunction, listOf(classPtr, LLVMGetArgOperand(call, 1)).toCValues(), 2, "")
