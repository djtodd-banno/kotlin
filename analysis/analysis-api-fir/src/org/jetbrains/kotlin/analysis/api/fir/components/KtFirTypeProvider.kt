...
internal class KtFirTypeProvider(
    ...
    private fun ConeLookupTagBasedType.getSubstitutedSuperTypes(shouldApproximate: Boolean): Sequence<ConeKotlinType> {
        ...
        // Skip substitution if type arguments do not match parameters.
        if (typeParameterSymbols.size != argumentTypes.size) return superTypes.asSequence()
    ...
    override fun getAllSuperTypes(type: KtType, shouldApproximate: Boolean): List<KtType> {
        require(type is KtFirType)
        val queue = type.coneType.getDirectSuperTypes(shouldApproximate).mapTo(ArrayDeque()) { it }
        return buildSet {
            while (queue.isNotEmpty()) {
                val current = queue.removeFirst()
                if (current in this) continue
                add(current)
